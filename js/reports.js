...
function renderDailyReport() {
    ...
    if (selDate) {
        ...
        dailySummary.innerHTML = `<div class="dsummary-card" style="grid-column: 1/-1; background: linear-gradient(135deg, rgba(185,28,28,0.08), rgba(225,29,72,0.04)); border: 1px solid rgba(185,28,28,0.15);"><div class="label" style="font-size:0.8rem; color:var(--primary); font-weight:700;">${dateLabel}</div></div><div class="dsummary-card"><div class="label">Penjualan</div><div class="value">${formatRupiah(dt)}</div></div><div class="dsummary-card"><div class="label">Keuntungan</div><div class="value green">${formatRupiah(dp)}</div></div><div class="dsummary-card"><div class="label">Pengeluaran</div><div class="value red">${formatRupiah(deAmt)}</div></div><div class="dsummary-card"><div class="label">Laba Bersih</div><div class="value" style="color:${net >= 0 ? 'var(--success)' : 'var(--danger)'}">${formatRupiah(net)}</div></div>`;
